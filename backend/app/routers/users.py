...
from backend.app.crud import create_user, get_user, list_users, update_user as crud_update_user
...
def update_user(user_id: int, payload: UserUpdate, db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    return crud_update_user(db, user, timezone_str=payload.timezone)
